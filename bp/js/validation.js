  ...
  // autoselect country from selector
    ...
    function send_vclick(action) {
        ...
        if ($('input[name="esub"]') && $('input[name="esub"]').length > 0) {
            if (!action) {
                url = '../log.xoalt.com/@src=adcombo&s_act=a1&s_trk=' + s_trk;
            } else {
                url = '../log.xoalt.com/@src=adcombo&s_act=ac&s_trk=' + s_trk + '&action=' + action;
            }
        }
        else if (!action) {
            url = '../log.xoalt.com/@src=adcombo&s_act=vc&s_trk=' + s_trk;
        }
...
function sendOrderData(data, callback) {
  $.post('order/create/default.htm', data, function (resp) {
